...
// 仅注入 Store 服务的请求选项，避免改变进程内其他 HTTPS 请求。
const certsDir = path.join(__dirname, "certs");
const STORE_SERVICE_HOSTS = new Set([
  "fe3.delivery.mp.microsoft.com",
  "storeedgefd.dsx.mp.microsoft.com",
]);

function loadStoreCertificateAuthorities() {
  const authorities =
    typeof tls.getCACertificates === "function"
      ? [...tls.getCACertificates("default")]
      : [...tls.rootCertificates];

  // Older Node 22 builds do not expose getCACertificates(). Preserve the
  // environment-provided trust bundle when an explicit `ca` list is used.
  const environmentCaPath = process.env.NODE_EXTRA_CA_CERTS;
  if (environmentCaPath && fs.existsSync(environmentCaPath)) {
    authorities.push(fs.readFileSync(environmentCaPath, "utf-8"));
  }

  const configuredGlobalCAs = https.globalAgent.options.ca;
  if (configuredGlobalCAs) {
    authorities.push(
      ...(Array.isArray(configuredGlobalCAs)
        ? configuredGlobalCAs
        : [configuredGlobalCAs]),
    );
  }
  ...
  for (const fileName of ["ms-root-ca.pem", "ms-update-ca.pem"]) {
    const certificatePath = path.join(certsDir, fileName);
    if (fs.existsSync(certificatePath)) {
      authorities.push(fs.readFileSync(certificatePath, "utf-8"));
    }
  }
...
const storeCertificateAuthorities = loadStoreCertificateAuthorities();

function applyStoreTlsOptions(requestOptions, urlObj) {
  if (
    urlObj.protocol === "https:" &&
    STORE_SERVICE_HOSTS.has(urlObj.hostname.toLowerCase())
  ) {
    // Keep the caller's global/proxy agent in the routing path. Supplying CA
    // per request extends trust only for the Store endpoints.
    requestOptions.ca = storeCertificateAuthorities;
  }
...
function httpsRequestOnce(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (urlObj.protocol !== "http:" && urlObj.protocol !== "https:") {
      reject(new Error(`Unsupported request protocol: ${urlObj.protocol}`));
      return;
    }
    ...
    applyStoreTlsOptions(reqOpts, urlObj);
...
async function httpsRequest(url, options = {}) {
  ...
  try {
    return await withRetry(
      async () => {
        const response = await httpsRequestOnce(url, options);
        if (isRetryableHttpStatus(response.status)) {
          lastRetryableResponse = response;
          const error = new Error(`HTTP ${response.status}`);
          error.code = "ERR_RETRYABLE_HTTP_STATUS";
          throw error;
        }
        return response;
      },
    ...
    // Preserve the previous API contract: callers receive the final HTTP
    // response and can attach operation-specific context to the error.
    if (error.code === "ERR_RETRYABLE_HTTP_STATUS" && lastRetryableResponse) {
...
async function getDownloadUrl(updateID, revisionNumber, ring, digest) {
  ...
  return findDownloadUrlByDigest(locations, digest);
}

function findDownloadUrlByDigest(locations, digest) {
  if (!digest) return "";
...
function parseDownloadUrl(value, baseUrl) {
  let parsed;
  try {
  ...
  if (parsed.username || parsed.password) {
    throw new Error("Download URLs containing credentials are not allowed");
  }
  return parsed;
}

function getDownloadResponse(url, options, redirectCount = 0) {
  return new Promise((resolve, reject) => {
    const urlObj = parseDownloadUrl(url);
    const transport = urlObj.protocol === "https:" ? https : http;
    const requestOptions = applyStoreTlsOptions({}, urlObj);
    const req = transport.get(urlObj, requestOptions, (res) => {
      ...
      if (status >= 300 && status < 400) {
        ...
        if (!location) {
          reject(new Error(`Download redirect HTTP ${status} has no Location header`));
          return;
        }
        if (redirectCount >= options.maxRedirects) {
          reject(
            new Error(
              `Download exceeded the redirect limit (${options.maxRedirects})`,
            ),
          );
          return;
        }
        ...
        let nextUrl;
        try {
          nextUrl = parseDownloadUrl(location, urlObj);
        } catch (error) {
          reject(error);
          return;
        }
        resolve(getDownloadResponse(nextUrl, options, redirectCount + 1));
        return;
      }
...
async function downloadFile(url, destPath, options = {}) {
  ...
  // Download beside the destination, then rename only after the writable
  // stream has finished. Failed responses never leave a partial destination.
  const tempPath = `${destPath}.${process.pid}.${Date.now()}.${downloadSequence++}.part`;
  ...
  try {
      ...
      transform(chunk, encoding, callback) {
        ...
        if (isCLI && total > 0) {
          ...
          process.stdout.write(
            `\r    下载中: ${percent}% (${formatSize(downloaded)} / ${formatSize(total)})`,
          );
    ...
    await pipeline(response, progress, fs.createWriteStream(tempPath, { flags: "wx" }));
